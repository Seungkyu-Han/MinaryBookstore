...
class ChatSocket(private val chatRepository: ChatRepository,
    ...
    private val chatWebSocketHandler: MutableList<ChatWebSocketSession> = CopyOnWriteArrayList()
    private val webSocketHandler: MutableList<WebSocketSession> = CopyOnWriteArrayList()
    ...
    override fun afterConnectionClosed(session: WebSocketSession, status: CloseStatus) {
        ...
        val iterator = chatWebSocketHandler.iterator()
        while (iterator.hasNext()) {
            val chatWebSocketSession = iterator.next()
            if (chatWebSocketSession.webSocketSession == session) {
                iterator.remove()
            }
        }
        ...
        if (isEstablished) {
            ...
            for (chatWebSocketSession in chatWebSocketHandler) {
                ...
                if (chatWebSocketSession.receiver.id == receiver && chatWebSocketSession.sender.id == senderId) {
                    try {
                        println("SEND MESSAGE TO $receiver FROM $senderId CONTENT${content}")
